...
**Rule**: put all custom activation shell code inside `system.activationScripts.postActivation.text`. `pro-darwin`'s single `postActivation` block contains: Maccy `defaults write`, and the `install_gcloud_component` helper + its calls.
...
- Not managed by Nix. Set Arc (or another browser) manually in **System Settings → Desktop & Dock → Default web browser**.
- Do **not** reintroduce `defaultbrowser` activation or PlistBuddy hacks on `com.apple.launchservices.secure.plist` — both caused fragile/broken behavior.
...
- **Do not** move `arc`, `warp`, `cloudflare-warp`, `tailscale-app`, `maccy`, `docker-desktop`, `whatsapp`, `handy`, `loom`, or `visual-studio-code` from Homebrew to Nix — they cannot work as Nix packages (see reasons above).
