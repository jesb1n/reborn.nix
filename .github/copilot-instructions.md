...
- The four Oracle micro nodes use `remoteBuild = false`; Mac-initiated builds use the configured `s145` distributed builder. Other deploy-rs nodes use `remoteBuild = true`. `rpi` uses `nixos-raspberrypi.lib.nixosSystem`; other NixOS hosts use `nixpkgs-unstable`.
